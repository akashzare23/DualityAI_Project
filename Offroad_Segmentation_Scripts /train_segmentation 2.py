...
class MaskDataset(Dataset):
    def __init__(self, data_dir, transform=None, target_size=(266, 476)):
        """
        data_dir: path containing 'Color_Images' and 'Segmentation' subfolders
        transform: torchvision transform for images (ToTensor + Normalize)
        target_size: (height, width) to resize images and masks
        """
        ...
        # Only keep image files (avoid .DS_Store, .txt, etc.)
        valid_ext = ('.jpg', '.jpeg', '.png', '.bmp', '.tiff')
        self.data_ids = [f for f in os.listdir(self.image_dir)
                         if f.lower().endswith(valid_ext)]

        # Mask resize transform (nearest neighbour to preserve class IDs)
        self.mask_resize = transforms.Resize((self.target_h, self.target_w),
    ...
    def __getitem__(self, idx):
        ...
        # Fix mask path: assume masks are .png files with same base name
        ...
        mask = convert_mask(mask)          # PIL with values 0..9

        # Resize both (bilinear for image, nearest for mask)
        ...
        # Convert mask to LongTensor directly (no ToTensor, no scaling)
...
# ============================================================================
# Metrics (optimized – no extra forward passes)
# ============================================================================

def compute_iou_from_logits(logits, target, num_classes=10):
    """Compute mean IoU given logits and target (LongTensor)."""
...
def compute_dice_from_logits(logits, target, num_classes=10, smooth=1e-6):
    """Compute mean Dice score given logits and target."""
    ...
    for class_id in range(num_classes):
        ...
        target_inds = target == class_id
        intersection = (pred_inds & target_inds).sum().float()
        dice = (2. * intersection + smooth) / (pred_inds.sum().float() + target_inds.sum().float() + smooth)
        dice_per_class.append(dice.cpu().numpy())
    return np.mean(dice_per_class)


def compute_pixel_accuracy_from_logits(logits, target):
    """Compute pixel accuracy given logits and target."""
    pred = torch.argmax(logits, dim=1)
...
# ============================================================================
# Main Training Function (FIXED: no double evaluation)
# ============================================================================
...
def main():
    ...
    # Transforms for images only (masks handled separately)
    ...
    # Create datasets (pass target size)
    trainset = MaskDataset(data_dir=data_dir, transform=transform, target_size=(h, w))
    train_loader = DataLoader(trainset, batch_size=batch_size, shuffle=True)

    valset = MaskDataset(data_dir=val_dir, transform=transform, target_size=(h, w))
    ...
    for epoch in epoch_pbar:
        ...
        for imgs, labels in train_pbar:
            ...
            # Compute metrics for this batch (optional but useful)
            iou = compute_iou_from_logits(outputs, labels, num_classes=n_classes)
            dice = compute_dice_from_logits(outputs, labels, num_classes=n_classes)
            acc = compute_pixel_accuracy_from_logits(outputs, labels)
        ...
        epoch_train_iou = np.mean(train_iou_list)
        epoch_val_iou = np.mean(val_iou_list)
        epoch_train_dice = np.mean(train_dice_list)
        epoch_val_dice = np.mean(val_dice_list)
        epoch_train_acc = np.mean(train_acc_list)
        epoch_val_acc = np.mean(val_acc_list)
        ...
        epoch_pbar.set_postfix(
            ...
            val_iou=f"{epoch_val_iou:.3f}",
            val_acc=f"{epoch_val_acc:.3f}"
        )
